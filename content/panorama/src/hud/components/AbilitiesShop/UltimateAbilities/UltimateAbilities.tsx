	...
	return (
		<Panel className={Styles.container}>
			<Panel className={Styles.titleContainer}>
				<Label text='Ultimate Abilities' className={Styles.title} />
				<Label text={`${nettable.length} / 1`} className={Styles.abilityCountLabel} />
			</Panel>
			<Panel className={Styles.abilitiesContainer}>
				{ultimateAbilities.map(ultimateAbility => {
					return <AbilityImage key={ultimateAbility.name} selectedUnit={selectedUnit} shopAbility={ultimateAbility} searchValue={searchValue} />
				})}
			</Panel>
		</Panel>
	)
