...
const SelectedEntity = () => {
	...
	return (
		<Panel className={Styles.container} hittest>
			<DOTAScenePanel id='selected_entity_portrait' className={Styles.heroImage} />
				...
				<Panel className={Styles.row1}>
					<Panel className={Styles.leftColumn}>
						<Panel className={Styles.leftColumnTitleContainer}>
							<Label className={Styles.heroLabel} text={$.Localize(`#${Entities.GetUnitName(selectedEntityIndex)}`)} />
						</Panel>
						<Panel className={Styles.leftColumnContentContainer}>
							<Panel className={Styles.columnContent}>
								{Entities.IsHero(selectedEntityIndex) && <Attributes selectedEntityIndex={selectedEntityIndex} />}
							</Panel>
						</Panel>
					</Panel>
