...
const MoveSpeed = (props: Props) => {
	...
	const [moveSpeed, setMoveSpeed] = useState(
		Entities.GetMoveSpeedModifier(selectedEntityIndex, Entities.GetBaseMoveSpeed(selectedEntityIndex)),
	)
	...
	return (
		<Panel className={ParentStyles.entry}>
			<Panel className={ParentStyles.imageContainer}>
				<Image src='file://{images}/icon_speed.png' className={ParentStyles.image} />
			</Panel>
			<Panel className={ParentStyles.labelContainer}>
				<Label className={`${ParentStyles.label} ${Styles.label} `} text={moveSpeed.toFixed(0)} />
			</Panel>
		</Panel>
	)
}
